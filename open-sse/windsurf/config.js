...
// Resolve a writable data directory shared with the 9router host process.
// Order: WINDSURF_DATA_DIR > 9router DATA_DIR > ~/.9router/windsurf
function resolveDataDir() {
  if (process.env.WINDSURF_DATA_DIR) return process.env.WINDSURF_DATA_DIR;
  if (process.env.DATA_DIR) return join(process.env.DATA_DIR, 'windsurf');
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA || join(os.homedir(), 'AppData', 'Roaming');
    return join(appData, '9router', 'windsurf');
  }
  return join(os.homedir(), '.9router', 'windsurf');
}
...
export const config = {
  ...
  // Data directory for accounts.json, stats.json, model-access.json — shared
  // with the 9router host so account state survives restarts of the parent app.
  dataDir: DATA_DIR,
};
